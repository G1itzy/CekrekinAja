            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($reservasi as $item)
                        <tr>
                            <!-- Null check for the order relationship -->
                            <td>
                                @if($item->order && $item->order->first()) 
                            ...
                            <td>
                                @money($item->total) &nbsp; 
                                <span class="badge bg-secondary">{{ $item->order->count() }} Alat</span>
                                @if ($item->status == 1)
            ...
            <table id="dataTable">
                ...
                <tbody>
                    @foreach ($riwayat as $r)
                        <tr>
                            <!-- Null check for the order relationship in history -->
                            <td>
                                @if($r->order && $r->order->first())
                            ...
                            <td>
                                @money($r->total) &nbsp; 
                                <span class="badge bg-secondary">{{ $r->order->count() }} Alat</span>
