                ...
                <div class="card-body" style="overflow: auto">
                    <table class="table table-success w-100">
                        <tbody>
                            <tr>
                                <th>No. Invoice</th>
                                <!-- Check if $detail->first() and $detail->first()->payment are not null -->
                                <td>
                                    @if ($detail->first() && $detail->first()->payment)
                                        {{ $detail->first()->payment->no_invoice }}
                                    @else
                                        <span class="text-danger">Invoice Tidak Tersedia</span>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                                <th>Penyewa</th>
                                <td>
                                    @if ($detail->first() && $detail->first()->user)
                                        <b>{{ $detail->first()->user->name }}</b> ({{ $detail->first()->user->email }})
                                    @else
                                        <span class="text-danger">Penyewa Tidak Tersedia</span>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    @if ($detail->first() && $detail->first()->user)
                                        {{ $detail->first()->user->telepon }}
                                    @else
                                        <span class="text-danger">Telepon Tidak Tersedia</span>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                                <th>Tanggal Pengambilan</th>
                                <!-- Check if starts exists -->
                                <td>
                                    @if ($detail->first() && $detail->first()->starts)
                                        {{ date('d M Y H:i', strtotime($detail->first()->starts)) }}
                                    @else
                                        <span class="text-danger">Tanggal Tidak Tersedia</span>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                        <form action="{{ route('acc', ['paymentId' => $detail->first() && $detail->first()->payment ? $detail->first()->payment->id : null]) }}" method="POST">
                            @method('PATCH')
                            @csrf
                    ...
                    <form action="{{ route('admin.penyewaan.cancel', ['id' => $detail->first()->payment->id]) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <button type="submit" class="btn btn-danger mb-3">Cancel Reservasi</button>
                    </form>                                      
                    @endif
                    @if ($status == 3)
                        ...
                        <div class="accordion-item">
                          ...
                          <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <div class="accordion-body">
                                @if ($detail->first()->payment && $detail->first()->payment->bukti == NULL)
...
<script>
    function confirmCancelReservation() {
        if (confirm('Anda yakin akan membatalkan reservasi?')) {
